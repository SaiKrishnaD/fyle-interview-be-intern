...
class Assignment(db.Model):
    ...
    def grade_assignment(cls, _id, grade, principal: Principal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, msg="NOT_FOUND")
        assertions.assert_valid(assignment.teacher_id == principal.teacher_id,"Submitted to other Teacher")
        assertions.assert_valid(not assignment.state == AssignmentStateEnum.GRADED, "Already graded can't be graded again")
        assertions.assert_valid(not assignment.state == AssignmentStateEnum.DRAFT, "It's still not Submitted")
        assertions.assert_valid(grade in ['A', 'B', 'C', 'D'], "Invalid")
